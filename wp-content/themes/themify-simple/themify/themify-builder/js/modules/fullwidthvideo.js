...
(function (Themify, document) {
    'use strict';
            ...
            Vimeo = function (el,url,id) {
				...
				let src = protocol+'//player.vimeo.com/video/' + id + '?api=1&byline=0&autopause=0&portrait=0&title=0&badge=0&controls=0&playsinline=1&callback=tb_vimeo&player_id=' + player_id,
					autoplay=false;
				if (params.loop) {
					src += '&loop=1';
				}
					...
					message=function(ev){
						if (ev.origin.indexOf('player.vimeo.com')!==-1) {
							...
							if(player_id===data.player_id){
								if(data.event==='ready'){
									const target=protocol+'//player.vimeo.com';
