...
class TB_Image_Module extends Themify_Builder_Component_Module {
	...
	<div class="module module-<?php echo $this->slug; ?> {{ fullwidth }} {{ classWrap }} {{ data.css_image }} <# data.appearance_image ? print( data.appearance_image.split('|').join(' ') ) : ''; #> tf_mw">
		...
		<a href="{{ data.link_image }}">
			...
			var link_image=data.link_image,
				ext=link_image.split('.').pop(),
			    icon=ext=='jpg' || ext=='jpeg' || ext=='png' || ext=='webp' || ext=='gif' || ext=='apng'?'ti-image':(link_image.indexOf('youtube')!=-1 || link_image.indexOf('vimeo')!=-1 || link_image.indexOf('youtu.be')!=-1 || ext=='jpg' || ext=='mp4' || ext=='f4v' || ext=='flv' || ext=='mov' || ext=='avi'?'ti-control-play':false);
			    if(icon==false){
